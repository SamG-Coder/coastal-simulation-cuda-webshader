...
try{
 ...
 await page.goto(`http://127.0.0.1:${server.address().port}/?profile${process.argv[3]==='legacy'?'&solver=legacy':''}`);
 ...
 result.mode=process.argv[3]==='legacy'?'GPU solver with CPU reconstruction/readback':'GPU resident';result.note='Exploratory browser-paced FPS; not completed-GPU throughput. See pipeline-performance.json for the controlled benchmark.';
